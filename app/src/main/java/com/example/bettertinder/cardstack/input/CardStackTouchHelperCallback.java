...
public class CardStackTouchHelperCallback extends ItemTouchHelper.Callback {
    ...
    public final void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        if (direction == ItemTouchHelper.LEFT)
        {
            ...
            if(CardStackAdapter.getPos()!=CardStackAdapter.MURDER)
                Log.i(TAG, "onSwiped: Right you killer");

            onItemSwiped.onItemSwipedRight(((CardStackAdapter.ItemView) viewHolder));
            onItemSwiped.onItemSwiped();
        }
    ...
    @Override public final float getSwipeThreshold(@NonNull RecyclerView.ViewHolder viewHolder) {
        return 0.5f;
    }
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (level > 0) {
                float scale = Math.max(0, Math.min(1,
                        (float) (1 - cardStackLayoutManager.getScaleGap() * level
